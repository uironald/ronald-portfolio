...
const nav = () => `
  <header class="site-header shell">
    ...
    <nav aria-label="Navegación principal">const email = 'mailto:roncoronald@gmail.com';
...
const cases = [
  { slug: 'propity', title: 'Propity', type: 'Producto inmobiliario', text: 'Una experiencia digital para ayudar a las personas a explorar, comparar y entender mejor sus opciones de vivienda.', className: 'case-visual propity-visual', image: '/assets/propity-detalle.png' },
  { slug: 'the-circle-club', title: 'The Circle Club', type: 'Marketplace para creadores', text: 'Una plataforma para conectar creadores de contenido con negocios y empresas.', className: 'case-visual circle-visual', image: '/assets/thecircleclub-detalle.png' },
...
function casesPage() {
  return `${nav()}<main class="shell page-main"><div class="page-heading">${sectionTitle('Casos de estudio')}<h1>Problemas reales, decisiones de diseño y productos listos para avanzar.</h1><p>Una selección de proyectos donde trabajé entre objetivos de negocio, necesidades de usuario y posibilidades de desarrollo.</p></div><div class="case-grid">${cases.map(c => `<a class="case-card" href="/casos/${c.slug}"><div class="${c.className}">${c.image ? `<img src="${c.image}" alt="Vista del caso ${c.title}" loading="lazy">` : ''}<span>${c.title}</span></div><div class="case-meta"><div><h2>${c.title}</h2><p>${c.type}</p></div><span class="arrow">↗</span></div><p>${c.text}</p></a>`).join('')}</div></main>${footer()}`;
}
